...
void Print_Message(void * SrcAddr, uint16_t dataSize)
{
    ...
    // Bound the copy to the buffer size instead of trusting SrcAddr to be
    // null-terminated within 100 bytes (previous strcpy() could overflow).
    uint16_t copy_len = dataSize;
    if (copy_len > sizeof(First_Data->Data) - 1)
    {
        copy_len = sizeof(First_Data->Data) - 1;
    }
    memcpy(First_Data->Data, SrcAddr, copy_len);
    First_Data->Data[copy_len] = '\0';
    First_Data->Data_Size = copy_len;
